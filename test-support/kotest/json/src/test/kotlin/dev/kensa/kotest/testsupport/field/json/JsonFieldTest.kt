...
class JsonFieldTest {
    ...
    fun `JsonField composes with of via core extension`() {
        val field = JsonTextField("/id")
        (field of "abc").test(sample).passed() shouldBe true
        val miss = (field of "xyz").test(sample)
    ...
    fun `JsonField composes with matching via core extension`() {
        val field = JsonTextField("/id")
        (field matching "[a-z]+").test(sample).passed() shouldBe true
    }
    ...
    fun `JsonMapKeyField extracts transformed value from map`() {
        ...
        nameKey.extract(map.mapValues { it.value }) shouldBe null
