...
class KensaExtension : Extension, ExecutionCondition, BeforeAllCallback, BeforeEachCallback, AfterTestExecutionCallback, InvocationInterceptor {

    /**
     * Called for every test element (class or method) before execution. Handles `@Disabled` class-level
     * detection by creating a [TestContainer] and writing output immediately, since [beforeAll] is never
     * called for disabled classes. Always returns enabled so Kensa never prevents test execution itself.
     */
    override fun evaluateExecutionCondition(context: ExtensionContext): ConditionEvaluationResult {
        if (context.testMethod.isEmpty && context.testClass.isPresent) {
            val testClass = context.requiredTestClass
            if (testClass.hasAnnotation<Disabled>() && context.kensaConfiguration.isOutputEnabled) {
                val container = context.kensaContext.createTestContainer(
                    testClass,
                    context.displayName,
                    TestPlanDetails.commonBasePackage
                )
                context.kensaResultWriter.writeTest(container)
            }
        }
        return ConditionEvaluationResult.enabled("Kensa does not control test execution")
    }
            ...
            CloseableKensaContext(
                ...
                KensaContext(
                    TestContainerFactory(
                        initialStateFor = { md ->
                            when {
                                md.hasAnnotation<Disabled>() -> Disabled
                                md is Method && md.declaringClass.hasAnnotation<Disabled>() -> Disabled
                                else -> NotExecuted
                            }
                        },
