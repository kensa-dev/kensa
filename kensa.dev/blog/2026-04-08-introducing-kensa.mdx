...
title: "BDD for Kotlin & Java Without the Gherkin Tax"
authors:
  - name: Paul Brooks
...
Here's a test from the [Clearwave example](https://github.com/kensa-dev/clearwave-example) — a fictional telecoms service that checks broadband feasibility across two supplier networks:
...
Add the dependency and implement `KensaTest`:

```kotlin
// build.gradle.kts
testImplementation("dev.kensa:kensa-framework-junit5:0.6.6")
```
...
The [Clearwave example repo](https://github.com/kensa-dev/clearwave-example) is a self-contained project showing a more complete scenario — async order processing across multiple suppliers, with sequence diagrams auto-generated from the interaction captures. You can browse the [live Kensa report](https://kensa-dev.github.io/clearwave-example) to see what the output looks like.
