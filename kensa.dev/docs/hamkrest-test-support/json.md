---
sidebar_position: 4
description: "Reference for the kensa-hamkrest-test-support-json module: JSON field classes backed by Jackson JsonNode and JSONPointer."
---
...
Source:
[`test-support/hamkrest/json/src/main/kotlin/dev/kensa/hamkrest/testsupport/field/json/`](https://github.com/kensa-dev/kensa/blob/master/test-support/hamkrest/json/src/main/kotlin/dev/kensa/hamkrest/testsupport/field/json/)

Extraction is backed by Jackson's `JsonNode` and JSONPointer (`JsonNode.at(path)`). No additional JSON library is required. The field classes import `MatcherField` from `dev.kensa.hamkrest.testsupport.field` — the only difference from the kotest counterparts is the package of the base interface.
...
```kotlin
open class JsonMapKeyField<TARGET>(override val keyName: String, private val fn: (JsonNode) -> TARGET?)
