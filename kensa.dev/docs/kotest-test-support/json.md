---
sidebar_position: 4
description: "Reference for the kensa-kotest-test-support-json module: JSON field classes backed by Jackson JsonNode and JSONPointer."
---
...
[`test-support/kotest/json/src/main/kotlin/dev/kensa/kotest/testsupport/field/json/`](https://github.com/kensa-dev/kensa/blob/master/test-support/kotest/json/src/main/kotlin/dev/kensa/kotest/testsupport/field/json/)

Extraction is backed by Jackson's `JsonNode` and JSONPointer (`JsonNode.at(path)`). No additional JSON library is required.
...
Most users will use a typed subclass rather than constructing `JsonField` directly. Reach for it when none of the typed subclasses fit — for example, to extract a computed value:
...
Declare these in a `object` or companion object. Each accepts a JSONPointer `path` string.
...
The lambda passed to `transformX` receives the already-extracted primitive; return `null` to signal a missing or invalid value.
