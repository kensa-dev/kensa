...
| ---------- | ------------------ | -------------- | ------------------------------------ |
| 0.9.19     | 0.9.6              | 0.9.6          | Built with Kotlin 2.4.20 — apply-time minimum consumer Kotlin is now 2.4.20. Default `kensaCoreVersion` bumped to 0.9.6 (log source manifest and MCP log tools #228; amber accent; dark mode follows the OS until chosen; chained call after an argument-less expandable #242). Min kensa-core raised to 0.9.6, as older cores carry a 2.4.10 compiler plugin; site assembly now requires `kensa-embed.js` and `favicon.png`. Stay on 0.9.18 until your project reaches Kotlin 2.4.20. |
| 0.9.18     | 0.9.5              | 0.8.0          | Site shell now copies `kensa-embed.js` and `favicon.png` from the core jar, so an aggregated site serves the embed sizer and the unfurl icon at its root (build-plugins #8). Cores older than 0.9.5 still assemble. |
| 0.9.17     | 0.9.5              | 0.8.0          | Default `kensaCoreVersion` bumped to 0.9.5 — report embed mode `#/embed/<testId>` with `theme=auto` and the `kensa-embed.js` iframe sizer (#224); link unfurl pages with Open Graph tags written under `embed/` when a `linkBaseUrl` is set; `withLinkBaseUrl` / `KENSA_LINK_BASE_URL` so copied links use the report's published address (#224); sticky test name in the report header (#223); filter by issue or epic from the badge right-click menu (#225). Site-mode reports pick up the matching UI shell. No plugin-side change; Kotlin floor stays 2.4.10, Gradle 9.7.0. |
| 0.9.16     | 0.9.4              | 0.8.0          | Default `kensaCoreVersion` bumped to 0.9.4 — `suite_summary` MCP tool and compact MCP listings (#221); `#/issue/<key>` and `#/epic/<id>` report links (#222); `SetupStep.setup(scope)` with an imperative `SetupScope` (#210), `SetupSteps(list)` (#213) and one-line `setupStep { }` / `setupActions(…)` builders (#212); polling blocks no longer repeat the wrapper keyword; deep links scroll the report's own content pane rather than an embedding page, and `?theme=dark|light` on a report URL sets the initial theme. Site-mode reports pick up the matching UI shell. No plugin-side change; Kotlin floor stays 2.4.10, Gradle 9.7.0. |
| 0.9.15     | 0.9.3              | 0.8.0          | Default `kensaCoreVersion` bumped to 0.9.3 — user thread locals carried into `thenEventually`/`thenContinually` checks via `withCoroutineContextProviders` (#220), hover anchors on test and invocation headers copy deep links (#219), MCP `captured_interactions` skips empty attribute groups (#218). Site-mode reports pick up the matching UI shell, including the deep-link anchors. No plugin-side change; Kotlin floor stays 2.4.10, Gradle 9.7.0. |
| 0.9.14     | 0.9.2              | 0.8.0          | Default `kensaCoreVersion` bumped to 0.9.2 — `run.json` run marker with live counts and the MCP run-state tools (`run_status`, `await_results`, `captured_interactions`, `failure_evidence`, rendered `get_test`) (#217); CLI wrapper arch detection, `KENSA_VERSION` pinning and checksum verification (#216); links naming both a test and an `issue:` filter keep the linked test (#215). Site-mode reports pick up the matching UI shell. No plugin-side change; Kotlin floor stays 2.4.10, Gradle 9.7.0. |
| 0.9.13     | 0.9.0              | 0.8.0          | Default `kensaCoreVersion` bumped to 0.9.0 — the API freeze ahead of 1.0. Implementation packages are now `internal` and core must pair with same-version adapters; the plugin does that pairing for you, so only explicit adapter pins need moving. 0.9.0 also brings `@Epic` (#194), the report overview page (#195), replay links on issue badges and fixes #185, #189–#193. Site-mode reports pick up the matching UI shell. No plugin-side change. |
| 0.9.12     | 0.8.16             | 0.8.0          | Built with Kotlin 2.4.10 — apply-time minimum consumer Kotlin is now 2.4.10; Gradle 9.7.0. Default `kensaCoreVersion` bumped to 0.8.16 (chained references through `@RenderedValueContainer`, the `by fixtures(fx)` property delegate, qualified enum constants #180, diagram glyph shapes #179). Stay on 0.9.11 until your project reaches Kotlin 2.4.10. |
| 0.9.11     | 0.8.15             | 0.8.0          | Default `kensaCoreVersion` bumped to 0.8.15, which adds the `kensa mcp` CLI server, negative matcher assertions and a sweep of expandable-sentence fixes. Pairs fine with a 0.9.9 or 0.9.10 compiler plugin too, since those already carry the 0.8.12 hook signatures (see 0.9.9 below). Set `kensaCoreVersion` to match your runtime, or pair via your version catalog (`kensaCoreVersion = libs.versions.kensa.get()`) to keep the two in lockstep. |
| 0.9.10     | 0.8.14             | 0.8.0          | Default `kensaCoreVersion` bumped to 0.8.14 — protected-phrase word-boundary matching (#166), parameterised header hover (#165), and the path-expression rendering sweep (#168–#172); 0.8.13's parser crash fix (#163) and `Named` argument labels (#162) included via the roll-up. No plugin-side change. |
| 0.9.9      | 0.8.12             | 0.8.0          | **Required for kensa-core 0.8.12+.** Default `kensaCoreVersion` bumped to 0.8.12, which fixes `@ExpandableSentence` / `@RenderedValue` with value class parameters and return types (#160) and array parameter types in parameterized tests (#161), and includes 0.8.11's multi-assertion `thenEventually` / `thenContinually` blocks. 0.8.12 also simplified the compiler-plugin hook signatures, so a pre-0.8.12 compiler plugin (0.9.8 and earlier, or a user-pinned `kensaCoreVersion` below 0.8.12) compiling against an 0.8.12+ `kensa` runtime fails with an internal compiler error (`IndexOutOfBoundsException: Index 3 out of bounds for length 3`). Take this release or later, or set `kensaCoreVersion` to match your runtime. |
| 0.9.8      | 0.8.10             | 0.8.0          | Default `kensaCoreVersion` bumped to 0.8.10, which adds [`@Fixture` factory functions](../api/factory-fixtures.md) (rewritten by the compiler plugin the Gradle plugin already applies — no config change) and fixes two `@RenderedValue` rendering bugs. Site mode also warns when a `sourceTitles` entry overrides a code-side title. Pairs only with kensa-core below 0.8.12 (see 0.9.9's hook-signature note). |
