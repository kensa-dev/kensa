...
| ---------- | ------------------ | -------------- | ------------------------------------ |
| 0.9.11     | 0.8.15             | 0.8.0          | **Required for kensa-core 0.8.15+.** The compiler-plugin hook signatures changed in kensa-core 0.8.15, so a pre-0.8.15 compiler plugin (the default in 0.9.10 and earlier) compiling against a 0.8.15+ `kensa` runtime fails with an internal compiler error (`IndexOutOfBoundsException` in `KensaIrGenerationExtension`). Take this release, or set `kensaCoreVersion` to match your runtime — pairing via your version catalog (`kensaCoreVersion = libs.versions.kensa.get()`) keeps the two in lockstep. 0.8.15 itself adds the `kensa mcp` CLI server, negative matcher assertions and a sweep of expandable-sentence fixes. |
