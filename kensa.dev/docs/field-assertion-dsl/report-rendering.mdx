...
```kotlin
val aFirstProfileType = XmlStringField(
    XPathExpressionWrapper(xpath.compile("/FeasibilityResponse/Profiles/Profile[1]/Type"), "/FeasibilityResponse/Profiles/Profile[1]/Type"),
    "Profile Type"
)
```

The hint reads the **string** preserved by `XPathExpressionWrapper`, not `XPathExpression.toString()`.
...
- **XPath fields without `XPathExpressionWrapper` produce no hint.** `XmlField.path` returns `null` if the underlying expression isn't wrapped, and Kensa renders no hint for `null`. Always wrap your compiled expressions if you want path hints.
