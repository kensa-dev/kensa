...
import dev.kensa.example.JavaTestInstance
...
internal class CachingScenarioMethodAccessorTest {
    ...
    @BeforeEach
    internal fun setUp() {
        accessor = CachingScenarioMethodAccessor(JavaTestInstance(), JavaTestInstance::class.allProperties.map { PropertyAccessor(it) }.associateBy { it.name })
    }
