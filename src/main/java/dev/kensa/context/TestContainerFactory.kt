...
class TestContainerFactory {
    ...
    private fun createInvocationData(method: Method): TestMethodInvocation {
        return TestMethodInvocation(
                method,
                deriveDisplayNameFor(method) { Strings.unCamel(method.name) },
                notesFor(method),
                issuesFor(method),
    ...
    private fun deriveDisplayNameFor(element: AnnotatedElement, lazyDefault: () -> String) = Reflect.findAnnotation<DisplayName>(element)?.value ?: lazyDefault()
